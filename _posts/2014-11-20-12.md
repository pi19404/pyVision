...
$\displaystyle x\_{T}(t) = \frac{1}{T} [ \int x(t)[\phi(2t)dt +\phi(2t-\frac{T}{2})]dt ]$

$\displaystyle  \phi(t) = \phi(2t)dt + \phi(2t-\frac{T}{2})$

Now if we consider interval over $\frac{T}{N}$ then we get ie $M=2^{N}$

$\displaystyle x\_{T}(t) = \frac{1}{2^N} \sum\_{i\in 2^N} x\_{i,\frac{T}{2^N}}(t) $

coming back again we can express difference of signal over the interval $\frac{T}{2}$

$e(t) = \frac{2}{T}\int\_{0}^{\frac{T}{2}} x(t)dt - \frac{1}{T}\int\_{T} x(t) dt $


This is called as wavelet function
$x\_{1,\frac{T}{2}}(t) = \frac{2}{T}\int x(t) \psi(t)$
...
Let $x[n]$ represent the discrete time sequence .This can be considered as projection coefficients on the $V\_{0}$ subspace.
